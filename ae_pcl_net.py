...
class ConvoLayer(nn.Module):
    def __init__(self):
        super(Conv0Layer, self).__init__()
...
class PrimePointCapsLayer(nn.Module):
    def __init__(self, prim_vec_size=8, num_points=2048):
        super(PrimePointCapsLayer, self).__init__()
...
class PointGenConv(nn.Module):
    def __init__(self, bottleneck_size=2500):
        ...
        super(PointGenConv, self).__init__()
...
class CapDecoder(nn.Module):
    def __init__(self, latent_caps_size, latent_vec_size, num_points):
        ...
        self.decoder = nn.ModuleList(
            [PointGenConv(bottleneck_size=self.bottleneck_size+2) for i in range(0, self.nb_primitives)])
    def forward(self, x):
        outs = []
...
class PointCapNet(nn.Module):
    def __init__(self, prim_caps_size, prim_vec_size, latent_caps_size, latent_vec_size, num_points):
        super(PointCapNet, self).__init__()
        self.conv_layer = ConvLayer()
        self.primary_point_caps_layer = PrimePointCapsLayer(prim_vec_size, num_points)
        self.latent_caps_layer = LatCapsLayer(latent_caps_size, prim_caps_size, prim_vec_size, latent_vec_size)
        self.caps_decoder = CapDecoder(latent_caps_size,latent_vec_size, num_points)
...
class PointCapNetDecoder(nn.Module):
    def __init__(self, prim_caps_size, prim_vec_size, digit_caps_size, digit_vec_size, num_points):
        super(PointCapNetDecoder, self).__init__()
        self.caps_decoder = CapDecoder(digit_caps_size,digit_vec_size, num_points)
